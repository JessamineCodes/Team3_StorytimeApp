...
def create(theme):
    if request.method == 'POST':
        ...
        story_id = story_manager.insert_mock_story(story_title, story_text, child_name, user_id)

        # Insert the story into the database
        # insert_story_query = "INSERT INTO stories (Title, Content, ChildName, UserId) VALUES (%s, %s, %s, %s)"
        # story_title = f"{child_name}'s {theme.title()} Story"
        # story_manager = StoryManager()
        # story_id = story_manager.execute_query(insert_story_query, (story_title, story_text, child_name, user_id))
...
def show_story(story_id):
    ...
    story = StoryManager.fetch_story_by_id(story_id)  # Fetch story using the fetch_story_by_id method
...
def show_saved_stories(user_id):
    ...
    stories = story_manager.fetch_all_user_stories(user_id)
